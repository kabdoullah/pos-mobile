...
- No hardcoded secrets — everything via `.env` (not committed). `APP_FRONTEND_URL` required in staging/prod (for verification/reset email links) — omitted from `.env.example`.
