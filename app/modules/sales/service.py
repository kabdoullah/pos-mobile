...
class SaleService:
    ...
    async def create_sale(
        self, store_id: UUID, payload: SaleCreate, user_id: UUID | None = None
    ) -> tuple[Sale, bool]:
        ...
        for item_payload in payload.items:
            ...
            if item_payload.product_id is not None:
                product = await self.product_repo.get_active_by_id(
                    item_payload.product_id, store_id
                )
                if product is None:
                    logger.warning(
        ...
        if was_created:
            resolved_items = [
                (item.product_id, item.quantity) for item in items if item.product_id is not None
            ]
            await self.inventory_service.record_sale_movements(
                store_id, created_sale.id, resolved_items, user_id
            )

        return created_sale, was_created
