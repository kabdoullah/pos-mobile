...
async def sync_sales_batch(
    ...
    """
    return await SyncService(db).sync_sales_batch(store_id, payload, user_id)
...
async def get_changes(
    ...
    return await SyncService(db).get_changes(
        store_id=store_id, since=since, cursor=cursor, limit=limit
    )
