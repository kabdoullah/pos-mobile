...
# ---------------------------------------------------------------------------
# Stock (traçabilité stock_movements) — CRITIQUE : décision produit, la vente
# ne bloque jamais sur stock insuffisant.
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_sale_decrements_stock_and_records_movement(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    """Vente sur produit suivi -> current_stock décrémenté, mouvement 'sale' tracé."""
    user = await _create_user(db_session, "stock-decrement@test.com")
    store = await _create_store(db_session, user.id)
    product = await _create_product(db_session, store.id, "Baguette", current_stock=10)
    await db_session.commit()
    ...
    assert str(movements[0].sale_id) == sale_id


@pytest.mark.asyncio
async def test_sale_allows_negative_stock(client: AsyncClient, db_session: AsyncSession) -> None:
    """Vente dépassant le stock disponible -> succès quand même, stock négatif.

    Régression la plus importante de la feature : le stock ne bloque JAMAIS une vente.
    """
    user = await _create_user(db_session, "stock-negative@test.com")
    store = await _create_store(db_session, user.id)
    product = await _create_product(db_session, store.id, "Baguette", current_stock=2)
    await db_session.commit()

    response = await client.post(
        "/api/v1/sales",
        json=_sale_payload(product_id=product.id, product_name="Baguette", quantity=5),
        headers=_headers(user.id, store.id),
    )
...
@pytest.mark.asyncio
async def test_sale_retry_does_not_double_decrement_stock(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    """Retry idempotent (même sale id) -> pas de second mouvement, pas de double décrément."""
    user = await _create_user(db_session, "stock-idempotent@test.com")
    store = await _create_store(db_session, user.id)
    product = await _create_product(db_session, store.id, "Baguette", current_stock=10)
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_sale_on_untracked_product_stays_untracked(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    """Vente sur produit non suivi (current_stock NULL) -> reste NULL, pas d'auto-activation."""
    user = await _create_user(db_session, "stock-untracked@test.com")
    store = await _create_store(db_session, user.id)
    product = await _create_product(db_session, store.id, "Baguette", current_stock=None)
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_download_sale_receipt_pdf(client: AsyncClient, db_session: AsyncSession) -> None:
    """GET /{sale_id}/receipt → 200, PDF valide avec en-têtes de téléchargement."""
    user = await _create_user(db_session, "receipt-ok@test.com")
    store = await _create_store(db_session, user.id)
    await db_session.commit()
    ...
    response = await client.get(f"/api/v1/sales/{sale_id}/receipt", headers=headers)
...
@pytest.mark.asyncio
async def test_download_sale_receipt_pdf_not_found(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    """GET receipt avec UUID inexistant → 404."""
    user = await _create_user(db_session, "receipt-404@test.com")
    store = await _create_store(db_session, user.id)
    await db_session.commit()

    response = await client.get(
        f"/api/v1/sales/{uuid4()}/receipt",
        headers=_headers(user.id, store.id),
    )
...
@pytest.mark.asyncio
async def test_user_a_cannot_download_receipt_of_user_b(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    """User A ne peut pas télécharger le reçu de la vente de B → 404."""
    user_a = await _create_user(db_session, "iso-a-receipt@test.com")
    user_b = await _create_user(db_session, "iso-b-receipt@test.com")
    store_a = await _create_store(db_session, user_a.id, "Store A")
    store_b = await _create_store(db_session, user_b.id, "Store B")
    await db_session.commit()
    ...
    response = await client.get(
        f"/api/v1/sales/{sale_id_b}/receipt",
        headers=_headers(user_a.id, store_a.id),
    )
    assert response.status_code == 404
